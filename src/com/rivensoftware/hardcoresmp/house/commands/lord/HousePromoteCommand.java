...
public class HousePromoteCommand extends HouseCommand
{
	...
	public void promoteCommand(CommandSender sender, String playerName) 
	{
		...
		{
			player.sendMessage(MessageTool.color("&cToo few arguments. &eUse like this: &b/h promote &3<player>"));
			return;
		} 
		...
		{
			player.sendMessage(MessageTool.color("&cYou cannot promote yourself!"));
			return;
		} 
		...
		{
			player.sendMessage(MessageTool.color("&c%PLAYER% is already an captain!").replace("%PLAYER%", name));
			return;
		} 
		house.getMembers().remove(uuid);
		house.getCaptains().add(uuid);
		house.sendMessage(MessageTool.color("&ePlayer &a%PLAYER%&e has been promoted to an captain by &f%LORD%").replace("%PLAYER%", name).replace("%LORD%", player.getName()));
	}
