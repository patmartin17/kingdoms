...
public class HouseAllyCommand extends HouseCommand
{
	...
	public void allyCommand(CommandSender sender, String houseName) 
	{
		...
		{
			player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.COMMON.NO_HOUSES_FOUND")).replace("%NAME%", houseName));
			return;
		} 
		else
		{
		...
		{
			player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.COMMON.ALREADY_HAVE_RELATIONSHIP")).replace("%HOUSE%", allyHouse.getHouseName()).replace("%RELATION%", "Ally"));
			return;
		} 
