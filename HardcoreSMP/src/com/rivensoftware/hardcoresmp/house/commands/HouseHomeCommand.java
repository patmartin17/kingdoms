...
public class HouseHomeCommand extends HouseCommand
{
	...
	public void homeCommand(CommandSender sender) 
	{
		...
		if ((getOrigArgs()).length >= 1 && player.hasPermission("kingdoms.admin")) 
		{
			...
			{
				player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.HOME.NO_HOUSE_FOUND")).replace("%NAME%", name));
				return;
			} 
		...
		{
			player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.HOME.CAPITAL_NOT_SET")));
			return;
		} 
		...
		else 
		{
			...
			player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.HOME.TELEPORTING")).replace("%TIME%", formatted + ""));
