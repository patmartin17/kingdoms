...
public class HouseInvitesCommand extends HouseCommand
{
	...
	public void invitesCommand(CommandSender sender) 
	{
		...
		player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.INVITES.YOUR_INVITES")).replace("%INVITES%", invites));
		...
		if (profile.getHouse() != null) 
		{
			...
			if (!house.getInvitedPlayers().isEmpty()) 
			{
				...
				player.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("COMMANDS.HOUSE.INVITES.HOUSE_INVITES")).replace("%INVITES%", invitedPlayers));
			} 
		} 
	}
}
