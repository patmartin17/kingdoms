	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) 
	{
		if (event.getEntity() instanceof Player) 
		{
			...
			if (damager != null && !damaged.getName().equals(damager.getName())) 
			{
				...
				if (profile.getProtection() != null) 
				{
					damager.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("LISTENERS.PROTECTION.NO_ACTION_PROTECTED")).replace("%TIME%", profile.getProtection().getTimeLeft()));
				...
				{
					damager.sendMessage(MessageTool.color(plugin.getLanguageConfig().getString("LISTENERS.PROTECTION.PLAYER_HAS_PROTECTION")).replace("%PLAYER%", damaged.getName()));
					event.setCancelled(true);
					return;
				} 
			} 
